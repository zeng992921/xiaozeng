...
public class GlobalExceptionHandler {
    ...
    public String errorHandler(Exception ex, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (ex instanceof IException) {
            return doHandler("error/500.html", ex.getMessage(), ex.toString(), request, response);
        } else if (ex instanceof UnauthorizedException) {
            return doHandler("error/403.html",  "没有访问权限", ex.toString(), request, response);
        }
        logger.error(ex.getMessage(), ex);
        return doHandler("error/500.html",  "系统错误", ex.toString(), request, response);
    }

    /**
     * 处理错误,ajax返回json，非ajax跳转页面
     */
    private String doHandler(String url,String msg, String error, HttpServletRequest request, HttpServletResponse response) {
        if (isAjax(request)) {
            cross(response);  // 支持跨域
            response.setContentType("application/json;charset=utf-8");
            try {
                PrintWriter out = response.getWriter();
                out.write(JSON.toJSONString(JsonResult.error(msg).put("error", error)));
                out.flush();
                out.close();
            } catch (IOException e) {
                e.printStackTrace();
